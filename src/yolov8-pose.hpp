...
public:
    ...
    void postprocess(std::vector<Detection>& objs, float score_thres = 0.25f, float iou_thres = 0.65f, int topk = 100);

    static void draw_objects(const cv::Mat&                                image,
                             cv::Mat&                                      res,
                             const std::vector<Detection>&                    objs,
...
void YOLOv8_pose::postprocess(std::vector<Detection>& objs, float score_thres, float iou_thres, int topk)
{
    objs.clear();
                               ...
                               const std::vector<std::vector<unsigned int>>& LIMB_COLORS)
{
    ...
    for (auto& obj : objs) {
        cv::rectangle(res, obj.bbox, {0, 0, 255}, 2);

        char text[256];
        sprintf(text, "person %.1f%%", obj.conf * 100);

        int      baseLine   = 0;
        cv::Size label_size = cv::getTextSize(text, cv::FONT_HERSHEY_SIMPLEX, 0.4, 1, &baseLine);

        int x = (int)obj.bbox.x;
        int y = (int)obj.bbox.y + 1;
